...
class NukeWriteGeoNode(tank.platform.Application):
    def init_app(self):
        """
        Called as the application is being initialized
        """
        # import module and create handler
        tk_nuke_writegeonode = self.import_module("tk_nuke_writegeonode")
        self.__write_node_handler = tk_nuke_writegeonode.TankWriteNodeHandler(self)

        # patch handler onto nuke module for access in WriteGeoNode knobs
        nuke._shotgun_writegeo_node_handler = self.__write_node_handler
        # and for backwards compatibility!
        nuke._tank_writegeo_node_handler = self.__write_node_handler

        # add WriteGeoNodes to nuke menu
    ...
    def destroy_app(self):
        ...
        """
        self.log_debug("Destroying tk-nuke-writegeonode app")
        ...
        if hasattr(nuke, "_shotgun_writegeo_node_handler"):
            del nuke._shotgun_writegeo_node_handler
        if hasattr(nuke, "_tank_writegeo_node_handler"):
            del nuke._tank_writegeo_node_handler

    def post_context_change(self, old_context, new_context):
        """
        Handles refreshing the render paths of all Shotgun write geo nodes
        ...
        # now the writegeonode handler settings have been updated we can update the paths of all existing PTR writegeonodes
    ...
    def process_placeholder_nodes(self):
        """
        Convert any placeholder nodes to TK Write Geo Nodes
        """
        self.__write_node_handler.process_placeholder_nodes()

    # interface for other apps to query write geo node info:
    #

    # access general information:
    def get_write_nodes(self):
        """
        Return list of all write geo nodes
        """
    ...
    def convert_to_write_nodes(self, show_warning=False):
        """
        Convert all Shotgun write geo nodes found in the current Script to regular
        Nuke WriteGeo nodes.  Additional toolkit information will be stored on
        ...
        # By default we want to convert the write geo nodes, unless the warning is shown and the user chooses to abort.
        ...
        if show_warning:
            ...
            res = QtGui.QMessageBox.question(
                None,
                "Convert All PTR Write Geo Nodes?",
                "This will convert all Flow Production Tracking write geo nodes to standard "
                "WriteGeo nodes."
                "\nOK to proceed?",
            ...
            if res != QtGui.QMessageBox.Yes:
                # User chose to abort the operation, we should not convert the write geo nodes
    ...
    def convert_from_write_nodes(self, show_warning=False):
        """
        Convert all regular Nuke WriteGeo nodes that have previously been converted
        from Flow Production Tracking WriteGeo nodes, back into Flow Production Tracking WriteGeo nodes.
        ...
        if show_warning:
            ...
            res = QtGui.QMessageBox.question(
                None,
                "Convert All WriteGeo Nodes?",
                "This will convert any Flow Production Tracking WriteGeo Nodes that have "
                "been converted "
                "into standard WriteGeo nodes back to their original form."
                "\nOK to proceed?",
                QtGui.QMessageBox.Yes | QtGui.QMessageBox.No,
            )
    ...
    def create_new_write_node(self, profile_name):
        """
        Creates a Shotgun write geo node using the provided profile_name.
        """
    ...
    def __add_write_node_commands(self, context=None):
        """
        Creates write geo node menu entries for all write node configurations
        and the convert to and from Shotgun write geo node actions if configured to do so.
        """
        ...
        if self.get_setting("show_convert_actions"):
            ...
            # todo: We should check the settings and then scan the scene to see if any PTR write geo nodes use promoted knobs
            ...
            if not promoted_knob_write_nodes:
                ...
                self.engine.register_command(
                    "Convert PTR Write Geo Nodes to WriteGeo Nodes...",
                ...
                self.engine.register_command(
                    "Convert WriteGeo Nodes back to PTR format...",
